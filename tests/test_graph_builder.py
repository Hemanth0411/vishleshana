...
def test_resolve_import():
    ...
    """
    all_files = ["/app/main.py", "/app/utils/helper.py", "/app/models/__init__.py"]
    ...
    # Nested match
    assert (
        graph_builder._resolve_import("utils.helper", all_files)
        == "/app/utils/helper.py"
    )

    # Init match
    assert (
        graph_builder._resolve_import("models", all_files) == "/app/models/__init__.py"
    )
